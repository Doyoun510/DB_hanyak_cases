...
const prescriptions = [
  ...
  { name: '당귀보혈탕', category: '보혈(補血)', ingredients: '황기, 당귀', efficacy: '기혈양허로 인한 피로·번열감·산후 회복에 활용.', description: '황기와 당귀의 비율 5:1. 이동원 『내외상변혹론』 출전.' },
  ...
  // 보양제(補陽劑)
  { name: '팔미지황환', category: '보양(補陽)', ingredients: '숙지황, 산약, 산수유, 택사, 목단피, 복령, 부자, 계지', efficacy: '신양허로 인한 요슬산연·하지냉증·야간뇨 완화.', description: '『금궤요략』 출전. 신기환이라고도 함.' },
...
async function main() {
  ...
  const pharmacist = await prisma.user.upsert({
    where: { email: 'pharm@test.com' },
    update: {},
    create: { email: 'pharm@test.com', password, name: '김한약사', role: 'PHARMACIST' },
  });

  await prisma.user.upsert({
    where: { email: 'user@test.com' },
    update: {},
    create: { email: 'user@test.com', password, name: '홍길동', role: 'USER' },
  });

  // 처방 (기존 항목도 약재/효능 등 최신 데이터로 갱신)
